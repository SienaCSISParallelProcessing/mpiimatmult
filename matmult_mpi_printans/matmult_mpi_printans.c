/* 
   ...
   Print out answer to a file "c.out"

   Jim Teresco, CS 400, Siena College
   2008-11-05

   Updated for Fall 2021, CSIS 335, Siena College
*/
...
int main(int argc, char *argv[]) {
  ...
  /* write the values of C to a file "c.out" */
  MPI_Barrier(MPI_COMM_WORLD);
  for (pid=0; pid<numprocs; pid++) {
    if (pid == rank) {
      FILE *fp = fopen("c.out", "a");
      for (row = 0; row < mysize; row++) {
	for (col = 0; col < size ; col++) {
	  fprintf(fp, "%10.4f ", c[row][col]);
	}
	fprintf(fp, "\n");
      }
      fclose(fp);
